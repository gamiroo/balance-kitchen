...
jest.mock('../form-field/FormField', () => {
  return function MockFormField({ children, label, helpText }: any) {
    return (
      <div>
        <label>{label}</label>
        {children}
...
describe('OrderStatusForm', () => {
  ...
  it('should render status field with correct label and help text', () => {
    ...
    expect(screen.getByText('Select the new status for this order')).toBeInTheDocument();
  });
  ...
  it('should prevent default form submission', () => {
    const mockSubmit = jest.fn((data: { status: string }) => {});
    render(<OrderStatusForm 
      orderId="ORD-001" 
      currentStatus="pending" 
      onSubmit={mockSubmit} 
  ...
  it('should handle different current status values with separate renders', () => {
    // Test pending
    const { unmount: unmount1 } = render(<OrderStatusForm 
      orderId="ORD-001" 
      currentStatus="pending" 
      onSubmit={mockOnSubmit} 
    />);
    ...
    // Test confirmed
    const { unmount: unmount2 } = render(<OrderStatusForm 
      orderId="ORD-001" 
      currentStatus="confirmed" 
      onSubmit={mockOnSubmit} 
    />);
    ...
    // Test delivered
    const { unmount: unmount3 } = render(<OrderStatusForm 
      orderId="ORD-001" 
      currentStatus="delivered" 
      onSubmit={mockOnSubmit} 
    />);
  ...
  it('should apply correct styling to disabled options', () => {
    ...
    const statusSelect = document.getElementById('status') as HTMLSelectElement;
    const disabledOption = statusSelect.options[0]; // Pending (Current)
  ...
  it('should not disable non-current status options', () => {
    ...
    const statusSelect = document.getElementById('status') as HTMLSelectElement;
    const enabledOption = statusSelect.options[1]; // Confirmed
