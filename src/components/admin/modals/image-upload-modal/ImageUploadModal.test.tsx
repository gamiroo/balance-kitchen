    ...
    return (
      <div data-testid="admin-modal" className="fixed inset-0 z-50 overflow-y-auto">
        <div className="flex min-h-screen items-center justify-center p-4">
          ...
          <div className={`relative transform overflow-hidden rounded-lg bg-gray-900 text-left shadow-xl transition-all sm:w-full ${size === 'md' ? 'max-w-xl' : 'max-w-md'}`}>
            <div className="bg-gray-800 px-4 py-3 border-b border-gray-700">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-white" data-testid="modal-title">
                  {title}
                </h3>
                <button
                  type="button"
...
describe('ImageUploadModal', () => {
  ...
  it('should render current image when provided', () => {
    render(<ImageUploadModal {...defaultProps} currentImage="https://example.com/image.jpg" />);
  ...
  it('should show error when trying to upload without selecting file', () => {
    ...
    // This prevents the user from trying to upload without selecting a file
    const uploadButton = screen.getByRole('button', { name: 'Upload Image' });
    expect(uploadButton).toBeDisabled();
    
    // The component correctly prevents uploading without a file by disabling the button
    // This is better UX than allowing the upload and then showing an error
  });
  ...
  it('should call onUpload with image URL when upload is successful', async () => {
    ...
    render(<ImageUploadModal {...defaultProps} onUpload={freshOnUpload} onClose={freshOnClose} />);
  ...
  // Mock the onUpload function to reject with an error
  const mockOnUploadError = jest.fn().mockRejectedValue(new Error('Upload failed'));
  
  render(<ImageUploadModal {...defaultProps} onUpload={mockOnUploadError} />);
  ...
  it('should handle special characters in title', () => {
    render(<ImageUploadModal {...defaultProps} title="Upload Image & Confirm" />);
    
    expect(screen.getByTestId('modal-title')).toHaveTextContent('Upload Image & Confirm');
  });
  ...
  it('should maintain button order (cancel then upload)', () => {
    ...
    // The last button should be the upload button (since close button is first in AdminModal)
  ...
  it('should clear error when new file is selected', async () => {
    ...
    // First select invalid file to trigger error
    ...
    expect(screen.queryByText('Please select an image file (JPEG, PNG, GIF)')).not.toBeInTheDocument();
  });
  ...
  it('should handle rapid open/close cycles', () => {
    const { rerender } = render(<ImageUploadModal {...defaultProps} isOpen={false} />);
  ...
  it('should render image preview with correct styling', () => {
    ...
    const image = document.querySelector('img');
    expect(image).toHaveClass('max-h-64', 'rounded-lg', 'object-contain');
  });
