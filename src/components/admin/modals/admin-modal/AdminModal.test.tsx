...
describe('AdminModal', () => {
  ...
  it('should render modal with correct title', () => {
    ...
    expect(screen.getByText('Test Modal')).toBeInTheDocument();
    expect(screen.getByText('Test Modal')).toHaveClass('text-lg', 'font-semibold', 'text-white');
  });
  ...
  it('should render close button by default', () => {
    ...
    expect(closeButton).toHaveClass('text-gray-400', 'hover:text-white', 'transition-colors');
  });
  ...
  it('should render with correct styling classes', () => {
    ...
    expect(modalWrapper).toHaveClass('fixed', 'inset-0', 'z-50', 'overflow-y-auto');
    ...
    expect(header).toHaveClass('bg-gray-800', 'px-4', 'py-3', 'border-b', 'border-gray-700');
  ...
  it('should handle empty children', () => {
    render(<AdminModal {...defaultProps} children={undefined} />);
  ...
  it('should prevent body scroll when modal opens and restore when closes', () => {
    const { rerender } = render(<AdminModal {...defaultProps} isOpen={false} />);
    
    expect(document.body.style.overflow).toBe('');
    
    rerender(<AdminModal {...defaultProps} isOpen={true} />);
  ...
  it('should handle multiple modals (only one should be active)', () => {
    ...
    // Both modals would render, but in practice only one should be open at a time
    expect(screen.getByText('Test Modal')).toBeInTheDocument();
  ...
  it('should apply backdrop blur effect', () => {
    ...
    expect(backdrop).toHaveClass('bg-black', 'bg-opacity-70', 'backdrop-blur-sm');
  });
  ...
  it('should render SVG close icon with correct attributes', () => {
    ...
    // The attributes might be kebab-case in the DOM
