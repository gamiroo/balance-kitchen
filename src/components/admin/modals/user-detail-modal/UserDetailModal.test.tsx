    ...
    return (
      <div data-testid="admin-modal" className="fixed inset-0 z-50 overflow-y-auto">
        <div className="flex min-h-screen items-center justify-center p-4">
          ...
          <div className={`relative transform overflow-hidden rounded-lg bg-gray-900 text-left shadow-xl transition-all sm:w-full ${size === 'lg' ? 'max-w-2xl' : 'max-w-md'}`}>
...
jest.mock('../../StatusBadge', () => {
  return function MockStatusBadge({ status, type }: any) {
    return <span data-testid="status-badge" data-status={status} data-type={type}>{status}</span>;
  };
});
...
describe('UserDetailModal', () => {
  ...
  it('should render role badge with correct styling for user role', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, role: 'user' }} />);
  ...
  it('should render role badge with correct styling for admin role', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, role: 'admin' }} />);
  ...
  it('should render account information with formatted dates', () => {
    ...
    // Check for the specific date elements
    const dateElements = screen.getAllByText((content, element) => {
      return content.includes('2023');
    });
  ...
  it('should render "Never" for last login when not available', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, last_login: undefined }} />);
  ...
  it('should render status toggle button with correct text and styling for active user', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, is_active: true }} />);
    
    const toggleButton = screen.getByRole('button', { name: 'Deactivate User' });
  ...
  it('should render status toggle button with correct text and styling for inactive user', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, is_active: false }} />);
    
    const toggleButton = screen.getByRole('button', { name: 'Activate User' });
    expect(toggleButton).toBeInTheDocument();
    expect(toggleButton).toHaveClass('bg-green-600');
  });

  it('should call onEditRole when role is changed', () => {
    render(<UserDetailModal {...defaultProps} onEditRole={mockOnEditRole} />);
  ...
  it('should call onToggleStatus when status toggle button is clicked', () => {
    render(<UserDetailModal {...defaultProps} onToggleStatus={mockOnToggleStatus} />);
  ...
  it('should not call onEditRole when not provided', () => {
    render(<UserDetailModal {...defaultProps} onEditRole={undefined} />);
  ...
  it('should not call onToggleStatus when not provided', () => {
    render(<UserDetailModal {...defaultProps} onToggleStatus={undefined} />);
  ...
  it('should handle user with no meal balance', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, meal_balance: 0 }} />);
    
    expect(screen.getByText('0 meals')).toBeInTheDocument();
  });

  it('should handle user with zero total spent', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, total_spent: 0 }} />);
    
    expect(screen.getByText('$0.00')).toBeInTheDocument();
  });

  it('should handle user with no orders', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, total_orders: 0 }} />);
  ...
  it('should call onEditRole with correct parameters for user role', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, role: 'admin' }} onEditRole={mockOnEditRole} />);
  ...
  it('should call onToggleStatus with correct parameters for activating user', () => {
    render(<UserDetailModal {...defaultProps} user={{ ...defaultProps.user, is_active: false }} onToggleStatus={mockOnToggleStatus} />);
